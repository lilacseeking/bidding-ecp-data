...
# 配置
# ============================================================
# --org 参数控制数据来源: sgcc(国家电网, 默认) / jibei(冀北)
_ORG_ARG = sys.argv[sys.argv.index('--org')+1] if '--org' in sys.argv else 'sgcc'
if _ORG_ARG == 'jibei':
    TARGET_ORG_ID = ORG_MAP["国网冀北电力有限公司"]
    DATA_SUBDIR = "data_jibei"
    OUTPUT_SUBDIR = "outputs_jibei"
else:
...
ZIP_DIR = os.path.join(PROJECT_ROOT, DATA_SUBDIR, "excels", "zip")
XLSX_DIR = os.path.join(PROJECT_ROOT, DATA_SUBDIR, "excels", "xlsx")
DB_PATH = os.path.join(PROJECT_ROOT, DATA_SUBDIR, "ecp_data.db")
FAILED_LOG = os.path.join(PROJECT_ROOT, DATA_SUBDIR, "failed_parse.json")
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            if col_map is None and len(rows) >= 3:
                # Fallback: 表头可能在第二行(合并单元格场景)
                headers2 = [str(c or '') for c in rows[1]]
                col_map = detect_column_map(headers2)
            if col_map is None:
